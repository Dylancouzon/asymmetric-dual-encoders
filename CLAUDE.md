...
- [ ] M10 (**PAUSED 2026-09-03 pending cloud GPU budget** — Dylan is on another project until then; resume from `m10/STATUS.md`. Retry of nano, planned 2026-09-01): same pair, same bars, new recipe built around the coverage finding: millions of synthetic queries in the six's forms and beyond, FineWeb breadth, LEAF's small-batch cyclic schedule, a registered ranking-aware phase-2 loss, warm start from the M9 candidate. Mandate `instructions-m10.md`; evidence `m10/PLANNING.md`; status `m10/STATUS.md`.
