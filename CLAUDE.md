...
- [ ] M13 (**blocked on M10**, was M12 until 2026-09-04): the nano half of the pair — release `constella-nano`, its ONNX port, its FastEmbed registration, and the whitepaper carrying the comparator table the model cards deliberately omit. `instructions-m13.md`. **The upstream FastEmbed PR is an M13 deliverable and waits for nano** (Dylan, 2026-09-04): one clean PR adding all three model entries, branched fresh from upstream `main`, not the throwaway `add-constella-models` branch.
- [ ] M14 (noted, not scoped — Dylan, 2026-08-30): an IMAGE model. Edge workloads are largely vision, so the asymmetric premise has a bigger market there. Open: whether the query side is a lookup table at all, image→image vs text→image, which frozen document tower. Do not inherit the text architecture's assumptions. `instructions-m14.md`.
- [ ] M15 (**parked, long-term, may never run** — Dylan, 2026-09-03): a better `zero` BEYOND the hybrid objective (that is M12). Idea register only, no compute committed: `instructions-m15.md`. The ranked lead is document-side co-adaptation (M8's `E14-LORA`, authorised and unrun), which reopens the frozen-tower premise and costs the drop-in-to-stella's-index property. The cheap lead is cosine-space distillation, priced at ~+0.035 by a pyNIFE retention comparison on fiqa (they retain 73.4% of a weaker teacher against our 67.1%).
...
- **Renumbering 2026-09-04 (Dylan), and the hybrid milestone:** `constella-zero-hybrid` is inserted as **M12**; nano's release M12→**M13**, the image model M13→**M14**, the better-zero idea register M14→**M15**. Historical renumbering entries above are left as written. Review-finding IDs named `M12`–`M15` in `m9/`, `m10/` and `research/` were NOT touched — they are findings, not milestones; the two E14-LORA pointers that did mean a milestone were rewritten to name M15 explicitly.
