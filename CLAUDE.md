...
- [ ] M10 (**budget VALIDATED and plan re-cut 2026-09-04**; resume from `m10/STATUS.md`. Retry of nano, planned 2026-09-01): same pair, same bars, same teacher, new recipe built around the coverage finding — a wider linear output from pooling three layers, LEAF's small-batch cyclic schedule, warm start from the M9 candidate, and query-form breadth from **≈1.5M harvested real titles/headings/claim sentences plus ≈1.0M generated queries** for the six forms no corpus contains (FineWeb ruled out of M10). **The 2026-09-04 efficiency review (amendments A1–A8, `instructions-m10.md` §Amendment 2026-09-04) cut the ranking-aware phase-2 class to one arm — LEAF's own ‖e‖₂ loss — deleting the candidate bank, the mining pass and the τ rule; cut generation 3.0M → 1.0M on three published saturation curves; withdrew the full-dose second seed; capped confirmations at two decisions; and re-registered C1/C2 on clean-4 as well as avg-6 (bars 0.5046 / 0.5233) so M14's headline partition carries a pre-registered pass/fail.** Compute re-priced on measured rates: the M10 recipe runs at **683 examples/s blended on the RTX 3080** (718 query-bucket / 596 document-bucket) against the plan's imported 560 for a rented A100 (the step is launch-bound at batch 32; `results/m10_rate_bench_box.json`, `m10/PLANNING.md` §11), so **the box runs the screens and the cloud buys generation and the build — ≈$110–280 against a $1,000 ceiling**, and the 2026-09-01 "not on a 3080" ruling now binds only the dose, not the hardware. **Reviewed four ways on 2026-09-04 (Fable ×2, Codex, Opus): amendments A1–A8 and B1–B6, every finding actioned; decisions 11 (release rule) and 12 (CUREv1 as a validation-only diagnostic) open for Dylan; `research/m10-feasibility-review-2026-09-04.md` has the feasibility verdict.** Mandate `instructions-m10.md`; evidence `m10/PLANNING.md` (§11 rates, §12 the synthetic-data question); status `m10/STATUS.md`.
