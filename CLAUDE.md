...
- [ ] M8 (**in progress**; the learnings-driven v2 of M7's zero-compute table — Dylan, 2026-08-28: "M8 will be the v2"). Tiny mandate in `instructions-m8.md`; **the binding protocol is `m8/LEDGER.md` and the executable half is `m8/registry.json` — read `m8/STATUS.md` first.** Pre-registered 2026-08-28 and still binding: M8's confirmatory sets are the RESERVED untouched-final four (FEVER, DBpedia-entity, cqadup-android/english), un-scored, one access; the M7-vs-M8 comparison is confirmatory only there; M7's six are development-informed (descriptive continuity only).
  **Re-routed 2026-08-29 by a milestone audit** (LEDGER §15): all nine probes so far returned nulls, negatives or instrument readings — data volume (`B3`), the doc-side head (`E14-HEAD`, −0.0244), the teacher (`T1`), the KL term (`B2`) and licensing (M7's clean-stack tax) are all closed — so the milestone now rests on **`D2`, a self-trained multi-word tokenizer trained through the forward**, registered with bar **0.00519** (the chain floor, adopted because D2 retrains the B leg). Its mechanism is §17b's fragmentation slope; its prior is not clean (VDR 30K→110K regressed 44.5→42.6). Named alternate if it misses: `B10`/`pool_mode`. **Bigram/n-gram rows are NOT a live lever** — §13 supersedes them with D2, no auto-revival. Everything else is deferred behind D2 with a `plan_status` on its registry row. **A pre-committed exit is in force**: if D2 and B10 both miss, M8 does not spend its access and closes as a measurement, leaving the reserved four clean for M9 — Dylan may override, but only before D2's number exists.
- [ ] M9 (queued, after M8 — renumbered from M8 on 2026-08-28): LEAF-style distilled small query tower against a frozen document index. **Teacher: M9 picks its own on measurement (Dylan, 2026-08-29: "M9 should go with whatever performs best, not as a strict continuation of M8") — default and re-probed incumbent stella_en_400M_v5, NOT bound to inherit M8's document tower, and T1's NO SWAP does not transfer because it is a fact about distilled TABLES, not towers; see m8/LEDGER.md §15** — tiny mandate in `instructions-m9.md` (the old `instructions-m8.md`, brought up to date: the bge-base teacher line and the shared-tokenizer rationale for a bge-small student were stale after the 2026-08-26 swap; student shortlist to be re-derived at M9 start). Release bar: CI-resolved above bge-small symmetric 0.5042. Comparator per-query vectors (leaf-ir-asym 0.5155, mdbr-leaf-ir 0.5123, arctic-m 0.5264) frozen into `results/perquery.json` 2026-08-25 while the Mac caches still existed.
