...
- [ ] M12 (**created 2026-09-04; next actionable, local box, 1–2 days, NO training**): **fusion operator audit.** M7's published `zero`+BM25 **0.4911** fuses with `convex0 w=0.8` per-query min-max — **which Qdrant does not ship**. Dev RRF k=10 is **0.5504** vs convex0 **0.5727**, a 0.022 gap larger than every table-side lever M7/M8 measured. DBSF is the untested third option, score-based, recommended by Qdrant beside RRF. Implement it to **Qdrant parity** (sample SD, `(s−(μ−3σ))/(6σ)`, no clipping, 0.5 for singleton/constant, depth 1000), score it against a **registered numeric rule**, dev-only and **descriptive — it does not replace the published headline**. `instructions-m12.md`; cut drafts in `m12/EXPLORED.md`. **Fusion-aware training (`constella-zero-hybrid`) was CUT here and moved to M16** — the trainer has no per-query candidate lists, both miners return IDs only and BM25 mines within-store, and the power arithmetic (fused seed SD 0.000332 → 3v3 SE ≈ 0.000271) made P(observed ≥ 0.008 | true 0.005) ≈ 0.
