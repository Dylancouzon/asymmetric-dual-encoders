...
- [x] M11 (**closed 2026-09-03**): the zero half of the pair, shipped. **`constella-zero`** — https://huggingface.co/DylanCouzon/constella-zero — and the frozen document tower **`stella-en-400M-v5-doc-onnx`** — https://huggingface.co/DylanCouzon/stella-en-400M-v5-doc-onnx — both public, ONNX, byte-verified, served by FastEmbed as built-in models on branch `Dylancouzon/fastembed@add-constella-models`. Serving parity vs the numpy reference **4.470e-08** on 1,024 dev queries. Mandate `instructions-m11.md` (Amendments A and B), detail `m11/STATUS.md`, reusable ONNX-port checklist `m11/CODEMAP.md`. Everything that depended on M10 moved to M13 (was M12 until 2026-09-04).
- [ ] M12 (**created 2026-09-04; next actionable, local box, NO training; Tier 1 is half a day**): **fusion operator audit.** M7's published `zero`+BM25 **0.4911** fuses with `convex0 w=0.8` per-query min-max — **which Qdrant does not ship**. But the 0.022 dev gap (RRF k=10 **0.5504** vs convex0 **0.5727**) is **not established as an operator gap — the comparison was unfair**: convex0 got a dev-fitted weight, RRF got none, and **Qdrant has shipped weighted RRF since v1.17.0** (Dylan caught this 2026-09-04). `fusion.rrf` has always taken `weights`; the M7 grid never used them. So M12 runs the fair comparison — RRF swept over `k` (M7's grid was monotone to its lower boundary and never reached Qdrant's default `k=2`), weighted RRF in **Qdrant's actual form** `1/((pos+1)/w + k − 1)` — NOT `w/(k+rank)`, which is what `fusion.rrf` implements — **and** DBSF — Implement it to **Qdrant parity** (sample SD, `(s−(μ−3σ))/(6σ)`, no clipping, 0.5 for singleton/constant, depth 1000), score it against a **registered numeric rule**, dev-only and **descriptive — it does not replace the published headline**. **Tiered: Tier 1 (convex0 reproduction gate + RRF over `k` + DBSF) answers the question; Tier 2 (weighted RRF, split-half) runs only if Tier 1 fails.** **This is a CLOUD finding** — server-side fusion is the only one-round-trip hybrid, whereas Qdrant Edge is embedded and a user can fuse in their own code; `bench/edge_prototype_pair.py` is dense-only, so edge fusion has never been built or costed and stays a **NOTE, not a required metric** (Dylan, 2026-09-04: edge numbers depend too much on hardware and corpus size). `instructions-m12.md`; cut drafts in `m12/EXPLORED.md`. **Fusion-aware training (`constella-zero-hybrid`) was CUT here and moved to M16** — the trainer has no per-query candidate lists, both miners return IDs only and BM25 mines within-store, and the power arithmetic (fused seed SD 0.000332 → 3v3 SE ≈ 0.000271) made P(observed ≥ 0.008 | true 0.005) ≈ 0.
- [ ] M13 (**blocked on M10**, was M12 until 2026-09-04): the nano half of the pair — release `constella-nano`, its ONNX port and its FastEmbed registration. `instructions-m13.md`. The whitepaper left for **M14** on 2026-09-04. **The upstream FastEmbed PR is an M13 deliverable and waits for nano** (Dylan, 2026-09-04): one clean PR adding all three model entries, branched fresh from upstream `main`, not the throwaway `add-constella-models` branch.
- [ ] M14 (**created 2026-09-04**, split out of M13): **the paper.** Runs after nano; runs anyway on the zero-only frontier if M10/M13 never do. **One paper is the default**, decided at M14's start. The deliverable is the frontier; the FINDING is that teacher quality does not predict distilled-table quality — Spearman ≈ 0 over eleven teachers, mxbai matching bge-base's tower (0.4433 vs 0.4484) with a table 0.057 worse — measured today, needing no nano. Carries the comparator table kept off the model cards. **Benchmark composition is decided HERE, once, for both zero and nano (Dylan, 2026-09-04) — but the RULE was registered 2026-09-04 before nano's numbers exist: headline = the pre-registered clean-4, all six reported beside it, contamination robustness as its own row, no re-picking the six, MS MARCO validation-only if built.** M12 measured why that is cheap: clean-4 costs the FUSED system 0.0045 and the dense table 0.0241. `instructions-m14.md`.
- [ ] M15 (noted, not scoped — Dylan, 2026-08-30): an IMAGE model. Edge workloads are largely vision, so the asymmetric premise has a bigger market there. Open: whether the query side is a lookup table at all, image→image vs text→image, which frozen document tower. Do not inherit the text architecture's assumptions. `instructions-m15.md`.
- [ ] M16 (**parked, long-term, may never run** — Dylan, 2026-09-03): a better `zero` BEYOND the hybrid objective (that is M12). Idea register only, no compute committed: `instructions-m16.md`. The ranked lead is document-side co-adaptation (M8's `E14-LORA`, authorised and unrun), which reopens the frozen-tower premise and costs the drop-in-to-stella's-index property. The cheap lead is cosine-space distillation, priced at ~+0.035 by a pyNIFE retention comparison on fiqa (they retain 73.4% of a weaker teacher against our 67.1%).
