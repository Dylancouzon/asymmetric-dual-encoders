...
- [ ] M12 (**created 2026-09-04; next actionable, local box, 1–2 days, NO training**): **fusion operator audit.** M7's published `zero`+BM25 **0.4911** fuses with `convex0 w=0.8` per-query min-max — **which Qdrant does not ship**. But the 0.022 dev gap (RRF k=10 **0.5504** vs convex0 **0.5727**) is **not established as an operator gap — the comparison was unfair**: convex0 got a dev-fitted weight, RRF got none, and **Qdrant has shipped weighted RRF since v1.17.0** (Dylan caught this 2026-09-04). `fusion.rrf` has always taken `weights`; the M7 grid never used them. So M12 runs the fair comparison — weighted RRF **and** DBSF — Implement it to **Qdrant parity** (sample SD, `(s−(μ−3σ))/(6σ)`, no clipping, 0.5 for singleton/constant, depth 1000), score it against a **registered numeric rule**, dev-only and **descriptive — it does not replace the published headline**. `instructions-m12.md`; cut drafts in `m12/EXPLORED.md`. **Fusion-aware training (`constella-zero-hybrid`) was CUT here and moved to M16** — the trainer has no per-query candidate lists, both miners return IDs only and BM25 mines within-store, and the power arithmetic (fused seed SD 0.000332 → 3v3 SE ≈ 0.000271) made P(observed ≥ 0.008 | true 0.005) ≈ 0.
