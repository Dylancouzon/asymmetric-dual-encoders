...
- [ ] M10 (**next — retry of nano, planned 2026-09-01**): same pair, same bars, new recipe built around the coverage finding: millions of synthetic queries in 12 forms seeded from Wikipedia and the approved corpora (FineWeb ruled out of M10), a wider linear output from pooling three layers, LEAF's small-batch cyclic schedule, a registered ranking-aware phase-2 loss, warm start from the M9 candidate. Mandate `instructions-m10.md`; evidence `m10/PLANNING.md`; status `m10/STATUS.md`.
