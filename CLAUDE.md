...
- [ ] M12 (**created 2026-09-04; next actionable, local 3080 only**): **`constella-zero-hybrid`** — does a fusion-aware objective beat the dense one for the same 31 MB table? Mandate rewritten the same day after Fable's review. **The question is the CANDIDATE SET, not the loss**: `zero`'s final phase is already InfoNCE (objective A), BM25 enters as a fixed additive logit bias, and M8 measured that objective **inert on the current candidates — positive ranked first for 99.75% of training queries** (`m8/FINDINGS.md` §3.1). Candidates must be union(dense top-k, BM25 top-k) or the arm is a known no-op. **Gate A first — DBSF on existing runs, CPU, ~1 h, no training**: it may close the 0.022 RRF-vs-convex0 gap by operator recommendation alone and be the whole milestone (RRF cannot be a training target — piecewise-constant, no gradient). Then two probes that CAN fail, then control-vs-hybrid × 3 seeds against a registered **0.008** bar — the repo's first recipe-replication term. **Does not spend the reserved four** (10.1M docs, tens of hours, P(ship | δ=0.005) = 0.21). `instructions-m12.md`.
