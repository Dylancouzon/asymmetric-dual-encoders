...
- [ ] M12 (**created 2026-09-04 by the renumbering below; next actionable, runs on the 3080**): **`constella-zero-hybrid`** — the same 31 MB int8 table, retrained against the FUSED ranking instead of stella's query vector, so it stops re-deriving lexical signal BM25 already supplies and spends capacity where BM25 is blind. Ships ALONGSIDE `constella-zero`, not instead of it: a fusion-trained table is deliberately worse standalone, and the fully-offline use case may not carry a local inverted index. `instructions-m12.md`. **Gate 0 before any training: measure the headroom** (fused-teacher vs fused-zero on dev, ~15 min, cached artifacts) and **choose the fusion operator the product can actually run** — the M7 headline uses `convex0 w=0.8` per-query min-max, which Qdrant does not ship; dev RRF is 0.5504 vs convex0 0.5727. Own pre-registration, own comparators (hybrid-vs-hybrid), confirmatory surface is the reserved four or LoTTE — **never the six**, whose fused-vs-dense rows were observed 2026-08-28.
