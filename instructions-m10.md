# M10 — release the pair, port it, and write the whitepaper (tiny mandate)

Created 2026-08-30 on Dylan's scoping. Runs after M9. Everything binds from `instructions-m7.md`
unchanged: decision authority, licensing and decontamination rules, dev-only selection, the
freeze/ledger protocol, and the headless git contract — working files under `m10/`.

M10 ships no new science. It turns two frozen artifacts into a product and a paper.
...
- The reserved four and their one confirmatory access carry forward unless M9 spends them.
