...
GPU budget (`CLAUDE.md` M10 entry). If M10 never runs, M13 does not run either, and the honest
...
*"The Fastembed PR should be filed under M13, once we have both models we add all of them in one
clean PR."* So the PR is an M13 deliverable and waits for nano, even though `constella-zero` and
...
So the M13 PR branches fresh from upstream `main` and carries only the model entries. What the zero
