# M15 — a better `constella-zero` (idea register)
...
only so no M15 plan forgets it while chasing the dense scalar.
...
- Student cap 35M is a hard rule (Dylan, 2026-09-01) and binds anything M15 adds on the query side.

## What would have to be true for M15 to run
...
1. **The frozen document tower is reopened.** Group A is the whole reason M15 would beat M7, and it
   costs the drop-in-to-stella's-index property. Without that, M15 is Group B and Group C.
2. **A GPU budget exists**, on the same footing as M10 (`instructions-m10.md` §Compute). M10 is
   already paused waiting for one, and M15 does not jump that queue.
3. **M10's status is settled first.** M10 is the nano retry on the same frozen tower. If the tower
   unfreezes, M10's recipe lock rests on a premise M15 discards, so the two are resequenced rather
