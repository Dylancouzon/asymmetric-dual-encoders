...
| `corpus_loader.py` | the M10 query corpora → the trainer: declared sources (M9 pool · PAQ · harvest · the generated half, unbuilt), the hashed manifest, packed pretokenization, the data cut, and the **form-balanced sampler** (`balanced=False` is A2's variant). `assemble_arm(arm_name, tok, student, ...)` is the ONLY function a launcher may use — a registered arm name or alias, never a source list; applies the cut, both M10 re-screen masks, the anchor/A4 12-form check and the cross-role guard | `results/m10_corpus_manifest.json` |
...
| `assemble10.py` | **step 8**: the per-form generation outputs -> the registered `generated_queries.jsonl`. Runs the screens the driver DEFERRED, in order and counted per form: rubric range, exact dedup, FORMS-12 **by document**, `protected10.hits` + the harvest's document streams, the own-seed span check (if the manifest says it is owed), A8 gate 1's action, the per-form quota (uniform, seed 0). `--data-cut` computes §0b's three unique-text counts and their `min` through `corpus_loader.load_segments` and **writes no registry**; `--gate2` re-runs A8 gate 2 for the forms present | `work/m10gen/generated_queries.jsonl`, `results/m10_assemble10.json`, `results/m10_data_cut.json` |
| `run_arm.py` | **the registered screen-arm runner**: one arm end to end from the registry — dose/batch/pattern/objective/student/head/warm start, corpus via `assemble_arm` ONLY, COV at every cycle end + family F's `read_at` points, DEV-6 once at the final checkpoint, retained per-cycle checkpoints, resume, and the arm's record. Refuses an invalid lock, an untrained or cut arm, a cut-corpus arm with no `data_cut` count, a record that already exists at EITHER published path, any recipe knob outside a smoke, and — for every arm after family F — a missing/stale `results/m10_F_verdict.json`, which is where a post-F arm's student comes from. A crash or an OOM writes a terminal FAILED record and re-raises; a REFUSAL writes none. `--plan` prices the W8-band-1 order; `--smoke-steps N` writes only under `work/m10arms/smoke/`. Contrasts are NOT computed here. It added two additive `trainer10` hooks: `read_steps` (extra reading points, kept OUT of the kill/plateau state) and `cycle_ckpt_fmt` (a retained checkpoint per cycle end). `--dev6 defer` (M13, 2026-09-10; refused under a smoke) records that DEV-6 was NOT read on this machine and names the final checkpoint bytes; `m13src/dev6_from_checkpoint.py` fills it on the box | `work/m10arms/<arm>/record.json` + `cov_*.json`, `results/m10_arm_<arm>.json` |
| `a8_gate2.py` | A8 gate 2: mean NN cosine (k=1/5) and two-sample energy distance of each query form against a 50K MS MARCO dev sample (validation-only, `work/m10msmarco/`); report-only, no action | `results/m10_a8_gate2.json` |
