...
| LEDGER (artefactory) | `finance` | `artefactory/ledger-long-context-KPI-QA` · `7881df56` (the collection's only QA member; there is no `artefactory/LEDGER` repo) | data CC BY 4.0, code MIT — licence is fine | 10,000 eval + 104,529 train rows ≈ the claimed 118K questions, but the fields are `query_id, report_full_text, query_text, answer_value, company_name, kpi, ticker, year` | not run — refused before any read | **REFUSED, on structure.** It is a long-context KPI-QA set, **not a retrieval benchmark**: no qrels and no corpus, the report inline on every row, and `answer_value` a number, so no passage-level relevance can be derived — only report-level. Reports run ~350K characters, so chunking is mandatory and the qrel would degrade to "any chunk of the right company-year report". Worse, **409 of 500 sampled queries contain their company name** and every one carries company and year, so the task reduces to company-name-plus-year matching. Admitting it would add a near-saturated component that compresses between-arm differences rather than resolving them |

**Family floor: PASSED, at the floor — and it stays at the floor.** Three family IDs admitted
(`consumer-health`, `BRIGHT`, `legal`); the registered STOP is "fewer than three". **`finance` does
not open: LEDGER is refused on structure** (row above), so there is no fourth family. No component
shows contamination against the six.
