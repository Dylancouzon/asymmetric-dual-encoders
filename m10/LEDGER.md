...
4. ~~The memmap guards test the READ artifact, not the WRITE path.~~ **CLOSED.** The writer is
   now pinned by observing each `write` to `flat.bin`: five chunks must produce five equal writes,
   and the accumulate-`np.concatenate`-write-once refactor the reviewers described produces a
   single 1,200-byte write and fails. (`st_size` cannot see this — the handle is buffered and the
   first version of the test asserted five zeros.) Also closed: `DOC_TEXT_CHUNK` is bounded at
   200,000 with a runtime `SystemExit` (it was settable to 5,000,000 with everything green), and
   `assemble_arm`'s inter-stream trim has an ORDER test, mutation-verified against both deleting
   it and moving it after the doc build.
...
| statistics | family-weighted COV macro over the four family IDs, paired stratified bootstrap over queries within unit, **B = 200,000, seed 0, `inverted_cdf`, one-sided α/12** — *the DENOMINATOR IS 12, not the 13 written here and in three places in the mandate: amendment C2 (`instructions-m10.md:160`) and `rules.F_orientation` both give 11 one-sided at α/12 + F1 two-sided at α/24 per tail = α exactly, which /13 does not. F1 was computed at α/24, so no number is wrong; the stale 13 is a trap (astra, 2026-09-08). Not moved after observation — 12 IS the registered value, 13 was never the operative one.*, **MDE 0.0056 fixed** | `statistics`, implemented in `m10src/cov_macro.py` |
...
| `health` | **2 of 2 — the last** | `8c30d65e`→`1a88671f`, sha256 `7d22a480b2687248b64274b3448bcf4389c6e67302b6dcbd79c2b529d421c1ad` (2026-09-05) | A8 gate 1 (W10-amended) **0.318 at n=2,000** on build seeds, above the 0.25 cut and rising | targets OPENER collapse (top-10 leading 4-grams = 34.5% of output, "what should i do" 13%): one distinct intent per list position — symptom / cause / treatment / risk / prognosis-or-when-to-see-a-doctor — no two questions sharing their first three words, "What should I do" at most once. Smoke and pilot: `work/m10gen/smoke/health_r2.json` |
...
**All four are pre-observation:** nothing registered has trained and no generation has run, so none
of this is a protocol change made after seeing the numbers it governs. The A8 amendment's
constant (50% of the smaller set) comes from the Jaccard intent, **not** from whatever value makes
`health` pass or fail — recorded because that is the way this decision could be corrupted.
...
| ~~W12~~ | **RULED 2026-09-05: STOP removed (§3 verification-pass rulings).** Original: **Family A's STOP rule can kill M10 for a surface mismatch — re-register it BEFORE family A runs.** `A3−A2` is the registered thesis contrast and its failure STOPS the milestone, but **COV contains none of A3's forms**: A3 adds titles, keywords and claim sentences (the clean-4 counterparts) while COV is long StackExchange posts, medical, contract and templated KPI questions. A3−A2 on COV is expected small **even if coverage is real**. A 5M dose compounds it — each query is seen ~once, and M9's failure is an asymptote over many epochs. **Recommended (reviewer, 2026-09-05): make family A descriptive triage on point estimates — STOP iff `A4−A2 point ≤ 0`** (the full corpus not even nominally beating the volume control is a genuine "something is wrong" signal), and **keep the generated half unless `A4−A3 point < 0`** (the current "unresolved → drop the 1.0M generated queries" discards 10 box-hours of correctly-labelled data on a test needing point > 0.011–0.014). All four A contrasts still reported with the registered bootstrap; only the ACTION changes, and it is labelled pilot-informed | Changes a registered outcome map and a registered STOP. Tier 3 | **must be settled before family A runs**; every A number is still unobserved, so it is pre-observation today |
| ~~W13~~ | **RULED 2026-09-05: L12 CUT (§3 verification-pass rulings).** Original: **L12's conditional extension is an adaptive selection step.** `F-MiniLM-L12` reaches 20M *because of* its own correlated 5M COV reading, and `F_selection_aware` recomputes only F1's winner inside the bootstrap. Counting F2 in the denominator whether or not it runs does not make the conditional 20M test valid. **Clean fix: ALWAYS extend L12** (+15M examples ≈ 4.5 box-hours, or the cloud); the alternative is a selective / group-sequential analysis nobody has specified. Raised by the W9 review, finding 6 | changes a registered dose and the F family's design. Tier 3 | before family F runs |
...
puts every arm on the box, so 32–39 h is wall-clock and days-to-lock, not budget; the cut question
is which arms can yield a verdict that changes the build, not which are expensive. I had framed it
as cost. (ii) **0.0043 is not a cliff**: power at a true 0.007 is 83% at d = 0.0043 and 77% at
d = 0.0056. The real discontinuity is **d > 0.0056**, where the effective threshold becomes d
itself and the 80%-power effect is 1.3·d. (iii) **P2's `distance_raw` brackets only B and D** —
two arms differing in one scalar are the most-correlated pair possible; **G, C and A will read
wider, so d is a FLOOR for them**, not their width. Also: the seed effect 0.001387 is **n = 1**
and must be reported that way (it implies a per-arm seed SD ≈ 0.0012, 3-seed range ≈ 0.002).

**The rule, keyed to `lr_pair.distance_raw` = d. Unconditional, whatever d is:**
**CUT C** — C-M9init at 5M starts from a model trained 3.74B tokens, so it wins its contrast **by
construction**, says nothing about 200M, is already disclosed as confounded with head init, and
would eat a confirmation slot. Build init = the registered default. **KEEP E** — 0.9 box-hours
(bs128 runs 1,517 ex/s) to decide a 37 h vs 81 h cloud build, and the 5M read is biased *toward*
bs32, so an unresolved E → bs128 is a safe saving. **KEEP F, ANCHOR, A.** **Do not touch
0.025/13** — fewer tails at the same per-tail level is strictly conservative, so cutting arms
needs no α re-registration. **Fix `lr_at` and re-smoke 90 steps before F**: a code change is a new
path.

| d | run | cut |
|---|---|---|
| **≤ 0.0056** | F · ANCHOR · A · G(384/1536/MLP) · B · E · D | C |
| **0.0056 < d ≤ 0.0086** | F · ANCHOR · A · G(all three) · E | B · D · C |
| **> 0.0086** | F · ANCHOR · A · G-384 · E | G-1536 · G-MLP · B · D · C — **and first DISTRUST THE CALIBRATION** (verify `total_steps` 156,250, `stopped` None, only LR differed) |

**THE FINDING THAT MATTERS — the thesis contrast is MIS-INSTRUMENTED, and the registered STOP can
kill M10 for a surface mismatch.** **COV contains none of A3's forms.** A3 adds titles, keywords
and claim sentences — the *clean-4* counterparts (scidocs↔titles, scifact↔claims). COV's queries
are long StackExchange posts (BRIGHT), medical questions, contract questions and templated KPI
questions. **So A3−A2 measured on COV is expected small even if the coverage thesis is TRUE**;
A4−A3 (health, finance, argument, how-to) is the contrast that actually maps onto COV. Compounding
it: at 5M each query text is seen ~once, and M9's coverage failure is an asymptote reached over
many epochs, so a 5M null is biased toward null; and the width is set by the surface (BRIGHT +
legal = 83% of variance), not by dose, so running A at 20M grows the effect but not the resolution.
...
Dylan ruled "fix and have it reviewed". The review's core criticism: `screen_lock.py` never
evaluated `family_rule`, `tails`, per-contrast `quantile` or `trained_arms_if_C_skipped`, and the
W9 tests asserted **that a sentence existed**. A misspelled `family_rule` or an ignored quantile
override passed the lock clean. **Same failure as the vacuous test caught earlier today** —
writing something down is not making it work.

**Fixed:**

| # | finding | fix |
|---|---|---|
| 2 | **E's action contradictory in three places, and a bs128 ADVANTAGE reads negative, cannot resolve, and the generic rule would have selected bs32 — the slower arm**, the opposite of A7's intent | `rules.E_cost` is now an explicit decision table and a registered **exception to `_default`**: bs32 iff E1 resolves, else bs128. Selecting bs128 is a NON-DEFAULT outcome and so is confirmation-eligible |
| 3 | **G1 not fixed — the registry carried BOTH readings**: `multi_arm_winner` called G's arms "alternatives" while the G1 note said 384 could never win | Picked one: **`G-384` is `selectable: false`**, a diagnostic arm, which is what the mandate's "the paper's evidence for the M9 diagnosis" makes it. A negative point is reported as evidence, **not an action** — my "CONTRADICTS the M9 diagnosis" overstated unresolved noise |
| 4 | **`multi_arm_winner` broke the α accounting** — "must also resolve against the other alternative" creates **five** post-hoc-oriented comparisons outside the thirteen | Among alternatives whose own default comparison resolves, the highest point wins; **no inferential claim against the others, no alternative-vs-alternative contrast run** |
| 5 | `rules.F_orientation` **still said α/13 per F tail**, contradicting the α/26 fields, and called it conservative when it put the familywise rate at 0.02885 | rewritten; and `_runner_contract` registers that **the runner MUST pass each contrast's own quantile** — `cov_macro.contrast` takes it, but the lock cannot see the call site |
| 7 | F's tournament underspecified: `F_selection_aware` defined only F2's comparator, not how F1 and F2 jointly produce "highest over runner-up resolves" | `rules.F_tournament` registered |
| 1 | the validator evaluated none of it | **it now does** — tails, per-contrast quantiles, the familywise sum, `family_rule` names, `selectable`, the conditional trained count. **Seven new tests MUTATE the registry and require refusal** rather than confirming prose |

**OPEN, and Tier 3 — the L12 extension is an adaptive selection step (finding #6).** L12 is allowed
to reach 20M *because of* its correlated 5M COV result, and `F_selection_aware` recomputes only
F1's winner. Counting F2 whether or not it runs does not make the conditional 20M test valid. **The
clean statistical fix is to ALWAYS extend L12** (+15M examples ≈ 4.5 box-hours); the alternative is
a selective or group-sequential analysis nobody has specified. → **W13**.

**Documented, not fixed** (all disclosed): the `runs_iff`/`skipped_iff` conditions are human-read
prose by necessity; `instructions-m10.md`:597 still says fifteen arms; "MDE" is annotated rather
than renamed; the confirmation-reversion rule is authoritative registry text but no reviewed code
applies it yet.
...
### Dylan's rulings, 2026-09-05 evening — verification pass (W12, W13, the cloud split). Pre-observation.

Raised after a lead-level check of the plan against the goal (aligned; the day had gone into
screen statistics while the corpus→trainer path stayed unbuilt; the screen no longer fit the window).

| # | ruled | executed as |
|---|---|---|
| **W12** | **Remove the hard STOP** (the recommended option; both reviewers agreed) | `rules.three_outcome` and `outcome_to_action.A` rewritten: three REPORTED labels, build proceeds in every case; family A also read on DEV-6 CQADupStack + FORMS-12, descriptive (`evaluation.family_A_diagnostics`). Failure condition = C1b. Mandate Tier-3 line and §Amendment 2026-09-05 C1 |
| **W13** | **Cut L12** (recommended; it is bge-small's size and depth, so F2 tested the checkpoint, not depth) | `F-MiniLM-L12` `cut`, F2 moved to `_cut_contrasts`, **n_contrasts 12**, generic quantile 0.025/12, F1 0.0125/12 (familywise exactly 0.025), `trained_arms_expected` 14; `F_tournament`/`F_orientation`/`outcome_to_action.F` rewritten for F1 only; `F_selection_aware` retired as a record. Mandate §Amendment 2026-09-05 C2; `test_screen_lock` updated |
| **cloud split** | *"I don't care if we go over the weekend timeline but also if things need to be delegated to cloud gpu it's fine. Though I prefer to do as much as we can here first"* | box first, no re-shaping; the remainder to the A100 under the same registry, both E arms there together. Mandate §Amendment 2026-09-05 C3 |

Also from the same pass (Dylan): the lead delegates build work to Opus/Sonnet subagents to keep
...
### Loader: third-pass items closed (Sonnet worker, …f079a8f + 9b3dec9), encode COMPLETE, A1 smoke PASS on CUDA — 2026-09-05 night

- **189 → 190 tests.** `corpus_loader.assemble_arm(name)` is the one launcher path (named arms only, cut + both masks + 12 forms for A4/ANCHOR + cross-role guard + `rescreen10.validate`). `results/m10_rescreen10.json` restored: queries **709** removed (pair 431 · nqopen 66 · triviaqa 212), documents **79,630**. Hold-out cache by digest; held-out `doc` ids refused (0 hits on the shipped 1,248,386 rows); stop state saved on a zero-step resume; tokenizer config in the cache identity.
- **Lead's reading, recorded:** the document draw for an arm = its document-EXAMPLE count (dose × pattern share: 1.25M for a 5M arm at 75/25), from the screened pool, so no document is presented twice at screen dose; M9 drew from every eligible row. The worker's 100K default would have repeated each document ~12×.
- **Teacher encode COMPLETE:** 5,243,386 unique texts (harvest + PAQ build + PAQ A2; 1,042,000 dropped as content-hash duplicates) at 488 texts/s, 2h59m. Cache `work/m10targets/query-stella_en_400M_v5-8e9a608b5383/`.
- **A1 CUDA smoke through `assemble_arm`: PASS** (`results/m10_arm_smoke_A1_cuda.json`): 403,443 queries after re-screen + dedup, 1,250,000 documents, cross-role collisions 0, 90 steps, 548 ex/s uncompiled at max_len 512. Cosmetic: the report HEADER echoes the CLI default `sources: harvest`; the per-arm record is authoritative.
- **12-shape smoke re-run on CUDA** (marker fix) in flight; the pre-fix record is kept at `work/m10arm_smoke_pre_marker_fix.json`. Codex fourth pass (GO/NO-GO) in flight.

### Codex FOURTH pass, 2026-09-05 night — NO-GO; 7 more closed; the lead REJECTS three of its remaining items
...
### Loader round two LANDED (Sonnet, 899daa3 · 48eaa6e · c69dc67), 2026-09-05 night — 206 tests; Codex fifth pass in flight
...
### GENERATION STARTED for the six approved forms, 2026-09-05 late evening (Opus worker); `health` r2 does NOT clear A8, both revisions spent

- **Health assertion re-run PASS:** contract 100% ×3, 954 out tok/s warm (`results/m10_gen_health_box_rerun.json`).
- **Diversity pilot** (`results/m10_diversity_pilot.json`): `finance` A8@2000 **0.0355** (cut 0.25) → started. Five more pilots in flight (PID 107433, `work/m10gen/pilot5.log`); the driver (`scripts/m10_gen_build.py`, PID 104715, `work/m10gen/build_driver.log`) pilots each form then builds it, and SKIPS a form whose pilot flags (reported, not resolved). Outputs `work/m10gen/generated_<form>.jsonl` (+ `.seeds` resume ledger, `manifest_<form>.json` with prompt sha, approval hash, seed-store identity, hold-out seed AND doc ids, per-screen removals).
- **ETAs at measured rates:** finance ≈1.3 h · comparison/yesno ≈1 h each · howto ≈2.5–3 h · conversational ≈3–3.5 h · **argument ≈6–8 h** (230 output tokens/query) → ≈15 h for the six, not the plan's ≈8.5. Accepted under Dylan's "timeline not binding".
- **Supply, reported not re-shaped:** finance 32,183 build seeds vs the 38,610 margin draw → ≈145K rows before the deferred screens, may land under the 143K quota; howto capped at 35,629 seeds (full-store supply).
- **Readings taken (Tier 2, change nothing registered):** the driver generates the full 1.35× margin and leaves the quota cut to step 8; protected-index / six's-documents screens and A8 run at step 8 on the manifest (only the rubric word range and own-seed 5-gram copy check run in the driver); **FORMS-12 for `wikipedia-body` is held by seed CHUNK (`article#chunk`) in `partition_seeds` — the mandate says seed DOCUMENTS, so step 8 closes the hold-out by `doc` (article) using the manifest's `forms12_holdout_docs`; the lead adopts this reading.**
- **`health` revision 2 of 2 (sha256 `7d22a480…c1ad`, LEDGER §1 row):** diagnosis = OPENER collapse (top-10 leading 4-grams 36.9% of r1 output; "what should i do" 16.2%). Smoke contract 100%. Pilot on the same build seeds, r1 control vs r2: @200 0.215 / **0.145**, @500 0.268 / 0.212, @1000 0.302 / 0.282, **@2000 0.318 / 0.3375**. Both far above the 0.25 cut and rising; r2 under-8-word drop 15.3% vs 8.2%. **Consequence is the REGISTERED A8 action, not a ruling:** at step 8 health is cut to its representatives; dropped only if < 50,000 remain. On-form judge (independent) running on the 50-query r2 sample; health generates after the judge and the six-hour veto window, so the A8 cut has something to act on.

### Codex SEVENTH pass — **GO** for training a screen arm through `assemble_arm` (1f67403, 211 tests), 2026-09-05 night
...
### Runner fixes LANDED (Opus, …6157453; 295 tests); **the M10.0-e calibration arms trained with dropout OFF** — disclosed. 2026-09-07 late afternoon

All thirteen closed with tests: `model.train()` before the loop and after every eval; bf16 autocast on CUDA with fp32 loss; AdamW wd 0.01 on dim>1 only; the **2,000-step cycle-1 warmup** in `lr_at` (clamped to cycle_1−2 on sub-warmup smokes only); a kill at the final cycle end is FAILED; real-run knobs and `--real-eval` removed from registered runs; the SHAPES check compares the untouched shape to the resolved recipe; evaluator records checkpointed; recipe fingerprint on every checkpoint (old checkpoints, incl. M10.0-e's, cannot be resumed — deliberate); crashes write a FAILED record; **post-F arms take their student from `results/m10_F_verdict.json`** (produced by the contrast step, not yet written) and refuse without it; COV surface asserted before scoring; atomic canonical record. Codex re-review in flight.
...
| 2026-09-05 | **M10.0-c — the M9 frozen candidate's DEV-6 baseline** (nq-250k, hotpotqa, cqadup-programmers, cqadup-physics, heldout-train, heldout-longq); checkpoint sha verified vs `m9/FREEZE.json` | full DEV-6 per-component + macro on the M9 candidate; no M10 candidate, no selection. Macro **0.556818** vs teacher 0.67238 = **82.8%**; per component 93.8 / 83.2 / 50.1 / 71.0 / 80.6 / 96.5% | `results/m10_m9_baseline_dev6.json`, `m10src/m9_baseline_dev6.py` |
...
`research/m10-opus-review-2026-09-04.md`. **Decision 12 (CUREv1 as a validation-only diagnostic) was ADOPTED ("yes") — but never EXECUTED; see the whole-plan review, miss M3.**
