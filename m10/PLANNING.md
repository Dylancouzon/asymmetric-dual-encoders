...
| Document-side co-adaptation (E14-LORA) | inside M10 it breaks the pair. Co-training the tower against **both** query paths at once keeps it, and it is the lever every ≥ 96% near-zero-query system used (LightRetriever's lookup 96% vs zero's 75.5%; ScalingNote 99%; CARE stage 2); costs a new index and a rebuild of zero | never inside M10; **recommended 2026-09-01 as the next-milestone candidate** (that slot was M12 then; it is **M15** after the 2026-09-04 renumbering), Dylan's call |
...
| 9 | co-training the tower recorded as the next-milestone candidate (now M15) | every ≥ 96% near-zero-query system co-trained the document side | §7 |
