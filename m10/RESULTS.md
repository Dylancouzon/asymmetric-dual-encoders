...
| **headroom H3 — seed supply** | `results/m10_seed_supply.json`, `m10src/seeds.supply()` | full-store scan, 5.35M documents, cross-store dedup: at identical stores and identical `min_score = 4`, the widened routing takes **health 10,399 → 42,380 (4.1×)**, finance 22,375 → 39,918, howto 37,927 → 37,154. All three clear the ~32–33K a 143K-query form needs, so the precision-costing rungs (relaxing `min_score`, raising queries-per-seed) are **not executed**. The shortfall was router recall, not corpus thinness | none |
| **headroom H1 — LEDGER** | `m10/LEDGER.md` §2 | **REFUSED on structure**: no qrels, no corpus, report-level relevance only, and 409/500 queries name their own company. The COV surface stays at three families and 3,416 queries, and §Surfaces' named remedy for the screen's power does not exist — W5 | none |
