...
| **5** PAQ | **DONE** | **A2 4,037,000** (`8f32bcdf…`) and **build 1,000,000** (`5bd7b728…`) nested inside it, from the official `dl.fbaipublicfiles.com` release (never an HF mirror), tarball `177eefb2…`, 64,875,601 pairs, CC BY-SA 3.0 shipped in the tarball. Screen removed only **0.35%** — PAQ does not reproduce NQ/TriviaQA phrasing. LEDGER §1 |
| **8 prep** | **step 8's gating logic now EXISTS** (it did not) | `m10src/corpus10.py` + 15 tests: the FORMS-12 hold-out (uniform, set aside BEFORE generation), the own-seed-passage 5-gram screen **with the harvested span's k-grams excluded** (without it every harvested string self-rejects), and A8 gate 1 at **16/32** — *not* the screens' 8/32. `corpus10.build_form` now composes the whole form pipeline and is tested end-to-end against a **stub OpenAI-compatible server**, so nothing but the model's own output quality is untested when the GPU frees. **Still to do:** generate FORMS-12's 500 eval queries per form from the held-out seed ids (`report["holdout_seed_ids"]`) — the seeds are held out of TRAINING correctly, but the eval set itself is a separate call — and assemble `results/m10_data_manifest.json` |
