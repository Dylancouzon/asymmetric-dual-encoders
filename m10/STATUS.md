...
| ~~5~~ | **DONE:** `results/m10_data_manifest.json` — 4.57 GB of corpus hashed on disk (raw pools included, not just the draw), 12 cited measurements, and it refuses to emit a complete-looking manifest with a hole. `m10src/data_manifest.py` | — | done |
| ~~6~~ | **DONE — DISCLOSED AS NOT RUN**, in the manifest's `disclosures`: near-vacuous by construction (the harvest is text COPIED from those documents, so it would report near-total overlap with its own source). The screen that matters, against the PROTECTED index, DID run (`results/m10_rescreen10.json`) and the disclosure names it | — | done |
