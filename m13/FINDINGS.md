...
R19 moved A100 execution—not its trigger, datasets, statistics, or interpretation—to M14. A later
owner instruction adds dense constella-zero to the descriptive roster through a dated amendment
that M14 must push before access; it does not alter the completed six-set gates. M13 did not resume
a GPU, create `m8-reserved-spent`, read reserved queries/qrels, invent a reserved score, publish to
Hugging Face, or open a FastEmbed PR. The full 18-dataset BEIR suite is also M14 pre-publication
descriptive validation and must not be retrofitted into M13's registered gates.
