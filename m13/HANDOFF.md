...
reserved query/qrel payload. A later owner instruction requires dense constella-zero in that
descriptive transaction. Before public release M14 must:

1. before access, push a dated registry/ledger amendment adding dense constella-zero, extend and
   test M13's three-system base executor, then run FEVER, DBpedia Entity, CQADupStack Android and
   English for Nano, dense Zero, BGE-small and LEAF in one zero-alpha transaction; Zero must reuse
   the Stella document vectors rather than authorize a second corpus-scale encode;
