...
registered work, not a failed or missing six-set result. A later owner instruction requires M14,
before any protected access, to amend the descriptive roster with dense constella-zero, extend and
test M13's three-system base package, and execute all four systems in the single transaction. The
separate broad descriptive BEIR-18 validation is also required before public release.
...
reserved datasets with Nano, dense Zero, BGE-small, and LEAF, the broad descriptive BEIR-18 run,
final packaging/parity checks, private upload, downloaded-byte verification, and the public
transition.
