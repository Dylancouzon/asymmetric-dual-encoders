...
| Pre-build gate | Registered (`m13/LOTTE_GATE_REGISTRATION.json`, amended 2026-09-10 under R17/R18); executor `m13src/lotte_gate13.py` (R16). Three review rounds (Astra nine findings, Sol ten, Astra's re-check three), all fixed (`m13/REVIEW_TRIAGE.md` §Stage 2); the closing re-check must return GO. **Before the read:** that GO, both E records pushed, the committed and pushed manifest and pin; the gate record is still owed |
