...
> **Pre-access amendment required:** add dense `constella-zero` to the reserved roster and extend
> the tested M13 three-system base package before running it. No reserved payload was opened in
> M13, so the single-access amendment remains admissible.
...
The terminal owner instruction adds a fourth system: released dense constella-zero as query
encoder with the same pinned Stella document tower used by Nano. Before any protected read, push a
dated registry/ledger amendment, update and test `reserved_support.py`,
`reserved_transaction.py`, `pre_encode.py`, and the cloud controller, and bind their new hashes to
the transaction. Zero must reuse Nano's Stella document shards; it does not authorize a second
corpus-scale Stella encode. Do not substitute Zero+BM25. If the access is already spent, preserve
the one-access rule and report Zero's omission rather than reopening it.
...
2. While access is still unspent, push the dated Zero registry/ledger amendment and extend/test
   the M13 base executor/controller. Then execute the amended controller on the retained A100.
   Require the exact four datasets and four systems (Nano, dense Zero, BGE-small, LEAF), durable
   `m8-reserved-spent` receipt, all atomic score outputs, `m13_reserved_run.json`, updated
   `m10_final_run.json` ending `COMPLETE`, cost receipt, and confirmed pod STOP. Treat every
   reserved comparison as zero-alpha descriptive evidence.
...
After the Zero amendment and implementation extension are committed, pushed, and tested without
protected access, run the amended reserved controller from the preserved artifact worktree at that
exact pushed `main` commit:
...
to `origin/main`, binds the transaction to that commit, and stops the paid pod in `finally`. The
committed M13 version is a three-system base and must not be executed unchanged; the amended M14
version must not be retargeted to a deleted M13 topic branch.
