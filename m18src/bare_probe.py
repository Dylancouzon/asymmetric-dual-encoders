...
def build(candidate_bundle, out, limit=10, candidate_name="candidate"):
    reg = registry()
    ...
    memory = system.ProjectMemory(WORK / "derived/index", candidate_bundle)
    bm25 = memory.bm25.run(terms, terms, 100)
    models = {"zero_v1": reg["models"]["zero_v1"]["source_path"],
              str(candidate_name): str(candidate_bundle)}
    result = {"_schema": "m18-bare-term-probe-v1", "report_only": True,
              ...
              "comparison_candidate": {"name": str(candidate_name),
...
def main(argv=None):
    ...
    result = build(args.candidate, args.out, args.limit, args.candidate_name)
