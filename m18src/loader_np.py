"""The standalone M18 numpy query loader: eager fp32 rows vs resident int8 codes/scales.
...
Array-size arithmetic is not an RSS measurement (m18/CODEMAP.md).
"""
...
class M18QueryEncoder:
    """numpy + tokenizers, no torch. `mode` selects the loading strategy under comparison."""

    def __init__(self, model_dir, variant=None, mode="resident_int8"):
        ...
        declared_variant = self.config.get("query_preprocessing", {}).get("variant", "T0")
        variant = declared_variant if variant is None else variant
        self.variant, self.mode = variant, mode
        if declared_variant != self.config.get("variant") or declared_variant != variant:
            raise ValueError(f"requested {variant} but bundle declares {declared_variant}")
        declared_impl = self.config.get("query_preprocessing", {}).get(
            "implementation_sha256")
        if declared_impl and declared_impl != preprocess.implementation_hash():
            raise ValueError("bundle preprocessing implementation differs from serving code")
        ...
        if mode == "eager_fp32":
            self.rows = self.codes.astype(np.float32) * self.scales[:, None]
    ...
    def encode(self, texts):
        ...
        if isinstance(texts, str):
            texts = [texts]
        texts = [preprocess.normalize_t1(t) if self.variant == "T1" else t for t in texts]
        out = np.empty((len(texts), self.dim), dtype=np.float32)
        for i, enc in enumerate(self.tokenizer.encode_batch(list(texts))):
            ids = preprocess.t2_mask_ids(enc.ids, self.tokenizer) if self.variant == "T2" else enc.ids
            out[i] = self._encode_ids(ids)
        return out
...
def parity(model_dir, texts=None, float_rows=None, tol=None):
    ...
    tol = tol if tol is not None else float(registry()["serving"]["loader_parity_max_abs"])
    texts = list(texts or DEFAULT_FIXTURES)
    cfg = json.loads(admit_read(Path(model_dir) / "config.json").read_text())
    variant = cfg.get("variant", "T0")
    a = M18QueryEncoder(model_dir, variant=variant, mode="eager_fp32")
    b = M18QueryEncoder(model_dir, variant=variant, mode="resident_int8")
    ...
    if float_rows is not None:
        ref = _reference_encode(np.asarray(float_rows, dtype=np.float32),
                                Path(model_dir) / "tokenizer.json", cfg, texts, variant)
...
def _reference_encode(rows, tokenizer_json, cfg, texts, variant="T0"):
    ...
    normed = [preprocess.normalize_t1(t) if variant == "T1" else t for t in texts]
    for i, enc in enumerate(tok.encode_batch(normed)):
        ids = preprocess.t2_mask_ids(enc.ids, tok) if variant == "T2" else enc.ids
        if not ids:
            out[i] = M18QueryEncoder._normalize(rows[fb_id])
            continue
        uniq, counts = np.unique(np.asarray(ids, dtype=np.int64), return_counts=True)
        w = np.sqrt(counts, dtype=np.float32)
        v = (rows[uniq] * w[:, None]).sum(0) / max(float(w.sum()), EPS)
        out[i] = (M18QueryEncoder._normalize(v) if float(np.linalg.norm(v)) > EPS
                  else M18QueryEncoder._normalize(rows[fb_id]))
    return out
