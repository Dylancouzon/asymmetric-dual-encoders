...
def eval_rows(W, tok, pre, V, bmap, components):
    """Full-suite eval of a stacked [unigram; bigram] row matrix via the bag/aug matrix path.
    With an empty bmap this is exactly the released table's forward (sum/len then normalize;
    folded weights live in the rows), verified against the gate numbers below."""
    per = {}
    for comp in components:
        doc_ids, _, q_ids, q_texts, qrels, dv = dev_eval.doc_vecs(comp)
        Xq = aug_matrix(tok, q_texts, pre, V, bmap) if bmap else bag_matrix(tok, q_texts, pre, V)
...
def main(k, lam=0.01, smoke=False):
    ...
    Waug = np.vstack([s * Wu, Wb.astype(np.float32)])
    comps = ["nq-250k", "cqadup-programmers", "cqadup-physics"] if smoke \
        else dev_eval.dev_components()
    per_base = eval_rows(Wu, tok, pre, V, {}, comps)
    per_aug = eval_rows(Waug, tok, pre, V, bmap, comps)
    q8, sc8 = quantize_int8(Waug)
    per_aug8 = eval_rows(dequantize_int8(q8, sc8), tok, pre, V, bmap, comps)
    qb8, scb8 = quantize_int8(Wu)
    per_base8 = eval_rows(dequantize_int8(qb8, scb8), tok, pre, V, {}, comps)

    out = {"k": k, "lam": lam, "n_queries": len(qs), "winner": WINNER.name,
