...
| **B3** | is Phase A pair-starved? | dense + fused macro on the registered OOD groups | ICT fractions {0, .25, .5, .75} at equal updates AND equal exposure | **B3 template**: sign-consistency on BOTH OOD components AND a seed replicate agreeing in sign, AND the group-vector median gain ≥ bar(§4.7) | Holm over the 3 non-zero arms at α = 0.05 | keep R0's ICT fraction (zero) |
