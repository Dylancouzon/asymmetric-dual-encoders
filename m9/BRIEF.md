...
- **Licensing:** clean stack; MS MARCO permanently excluded from the release stack (priced at
  +0.0058, declined commercially). CC BY-SA sources approved with model-card attribution. Vendor
...
| Serving-exact query encoder + bag machinery (reproduces the released path to 4e-08) | `m8src/d2_pre.py` |
...
- **When a floor lands, sweep every row whose bar reads TBD in the same commit.** Three probes sat
  unrunnable for all of M8 because this was not done, which made one lever look like the only route
  when it was merely the only *permitted* one.
