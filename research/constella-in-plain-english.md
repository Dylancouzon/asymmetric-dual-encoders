...
- **The LoTTE gate** reads the seven cleaned slices once, before the build. If the cloud arms select batch 128 it compares that checkpoint against the batch-32 one on fresh out-of-domain data and can veto in favour of 32. Its first independent review returned nine defects, all fixed the same day: an exclusive receipt so a crashed read cannot silently repeat, a committed checkpoint manifest and a per-slice hash pin so a swapped record or altered slice cannot pass, and checkpoint bytes hashed and loaded from one buffer. A second independent review decides GO.
...
**Where it stands.** Code complete and tested, 160 M13 tests plus the older suites all green. The provider and account are Dylan's to set up.
