...
| Kubernetes documentation (kubernetes/website) | ~thousands of pages | Content CC BY 4.0 per repo LICENSE; code samples Apache 2.0 | **NOT YET VERIFIED** — https://github.com/kubernetes/website/blob/main/LICENSE to be read and quoted | TBD | TBD vs dev suite and protected surfaces | **M17 proposal, admitted in principle by Dylan 2026-09-11 (m17/LEDGER.md A2), NOT admitted for download.** Before any download record here: exact repo revision and acquisition route (git clone of the repository, not a scrape of the rendered site), the content licence and its exceptions, the attribution artifact that will ship with derived weights, any acquisition terms restricting training (none are known for a plain git clone, unlike the StackExchange dump clickwrap; confirm), and decontamination coverage against the dev suite and protected surfaces via the approved fingerprint interfaces. Same checklist applies to any second project-documentation source. |
