#!/usr/bin/env python3
"""Restart the verified retained A100 after preflight-only failure and run the unchanged registered 200M build."""
import hashlib
import json
import math
import os
...
def main():
    ...
    import m13_preflight_retry_allocation as after_cpu
    global RESULT, ALLOC, BACKUP
    RESULT=REPO/'results/m13_cloud_build_preflight_retry.json'
    ALLOC=after_cpu.OUTPUT
    BACKUP=REPO/'work/m13cloud-build-preflight-retry-backup'
    log_path=REPO/'logs/m13-build-preflight-retry.log'
    ...
    try:
        ...
        if any(p['desiredStatus'] != 'EXITED' for p in pods):
            raise RuntimeError('Require all retained Pods stopped before preflight retry')
        ...
        pod=api()
        if pod.get('desiredStatus')!='EXITED' or not 0<=float(pod['costPerHr'])<=1.59:
            raise RuntimeError('Require stopped admitted-price retry target')
        ...
        if float(pod['costPerHr'])+530*.1/720 > price+1e-9:
            raise RuntimeError('Live GPU plus storage exceeds allocated price')
        live_target,_=after_cpu._live_target_capacity()
        machine=live_target.get('machine') or {}
        if (live_target.get('desiredStatus')!='EXITED' or machine.get('gpuAvailable',0)<1
            or machine.get('gpuTypeId')!='NVIDIA A100-SXM4-80GB'):
            raise RuntimeError('Original E host must have a free A100-SXM4-80GB')
        receipt['migration_ready_sha256']=sha(REPO/after_cpu.READY)
        started=True; launch_monotonic=time.monotonic(); save('resuming')
        ...
        if resumed.get('id')!=pod_id or resumed.get('gpuCount')!=1:
            raise RuntimeError('GPU retry did not return exact one-GPU target')
        ...
        if (live_target.get('desiredStatus')!='RUNNING' or live_target.get('gpuCount')!=1
            ...
            raise RuntimeError('Retry target must be one live A100-SXM4-80GB')
        save('deploying')
